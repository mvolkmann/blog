...
function getElementById(t, e) {
  ...
  if (s)
    for (const t of Array.from(s.children)) {
      const s = getElementById(t, e);
      if (s) return s;
    }
  for (const s of Array.from(t.children)) {
    const t = getElementById(s, e);
    if (t) return t;
  }
  return null;
}
...
function updateAttribute(t, e, s) {
  if (isPrimitive(s) && 'boolean' != typeof s) {
    t.getAttribute(e) !== s && t.setAttribute(e, String(s));
  } else {
    t[Wrec.getPropName(e)] = s;
  }
}
...
class Wrec extends HTMLElement {
  ...
  attributeChangedCallback(t, e, s) {
    const r = Wrec.getPropName(t),
      o = this.#l(r, String(s));
    (this[r] = o), this.#h(r, String(o)), this.propertyChangedCallback(r, e, s);
  }
  #u(t, e, s, r) {
    t.addEventListener(r, t => {
      const s = t.target,
        {value: r} = s,
        {type: o} = this.#r.properties[e];
  ...
  #d() {
    ...
    for (const [t, r] of Object.entries(s)) this.#R(t, r, e);
  }
  #R(t, e, s) {
    const r = Wrec.getAttrName(t),
      o = this.hasAttribute(r);
    ...
    (this[c] = a),
      ...
      Object.defineProperty(this, t, {
        ...
        set(s) {
          n === Number && 'string' == typeof s && (s = stringToNumber(s));
          const o = this[c];
          if (s === o) return;
          this.#v(t, n, s), (this[c] = s);
          const {state: i, stateProp: a} = this.#r.properties[t];
          a && (i[a] = s),
            this.#x(t),
            this.#S(t, n, s, r),
            this.#A(t),
            this.#N(t, s),
            isPrimitive(s) && this.#h(t, s),
            this.propertyChangedCallback(t, o, s),
  ...
  #F(t) {
    ...
    if ('style' === e) {
      ...
      if (s) {
        const r = this.#I(t, s);
        'textarea' === e && r
          ? (this.#u(t, r, null, 'change'), (t.textContent = this[r]))
          : this.#_(s, t);
      }
    }
  }
  ...
  #I(t, e) {
    ...
    return void 0 === this[s] && this.#$(t, '', s), s;
  }
  #A(t) {
    const e = this.#r.propToExprsMap.get(t) || [];
    for (const t of e) {
      const e = this.#y(t),
        s = this.#o.get(t) ?? [];
      for (const t of s)
  ...
  #P(t, e) {
    const {computed: s, uses: r} = e,
      o = this.#r.propToComputedMap;
    function n(e, s) {
      let r = o.get(e);
      r || ((r = []), o.set(e, r)), r.push([t, s]);
    }
    const i = s.match(REFS_RE) || [];
    for (const e of i) {
      const r = e.substring(5);
      void 0 === this[r] && this.#$(null, t, r),
        'function' != typeof this[r] && n(r, s);
    }
    if (r) for (const t of r.split(',')) n(t, s);
  }
  #_(t, e, s = void 0) {
    if (!t) return;
    const r = this.#O(e, s, t);
    if (!r) {
    ...
    r.forEach(e => {
      ...
      let n = r.get(s);
      n || ((n = []), r.set(s, n)), n.includes(t) || n.push(t);
    });
    for (const [t, e] of this.#o.entries())
      for (const s of e) {
        const r =
          s instanceof HTMLElement || s instanceof CSSStyleRule ? s : s.element;
    ...
    let n = this.#o.get(t);
    n || ((n = []), this.#o.set(t, n)),
      n.push(s ? {element: e, attrName: s} : e);
    const i = this.#y(t);
    s ? updateValue(e, s, i) : this.#L(e, i);
  }
  #h(t, e) {
  ...
  static #T() {
    for (const [t, e] of Wrec.#e.entries()) {
      const s = getElementById(document.body, t);
      if (s) for (const [t, r] of Object.entries(e)) s[t] = r;
    }
  }
  #M(t, e, s) {
    const r = this.#r,
  ...
  #$(t, e, s) {
    this.#M(t, e, `refers to missing property "${s}"`);
  }
  #C(t, e) {
  ...
  #l(t, e) {
    ...
    return (
      r || this.#M(null, t, 'does not specify its type'),
      r === String
        ? e
        : r === Number
        ? stringToNumber(e)
  ...
  #S(t, e, s, r) {
    if (isPrimitive(s) && this.hasAttribute(r)) {
      s !== (e === Boolean ? this.hasAttribute(r) : this.#C(t, r)) &&
    ...
    for (const t of s)
      if (t instanceof HTMLElement)
        ...
        if (s instanceof HTMLElement) {
          s[Wrec.getPropName(r)] = e;
        } else s instanceof CSSStyleRule && s.style.setProperty(r, e);
      }
  }
  #x(t) {
    const e = this.#r.propToComputedMap.get(t) || [];
    for (const [t, s] of e) this[t] = this.#y(s);
  }
  #L(t, e) {
    if (void 0 === e) return;
  ...
  useState(t, e) {
    for (const [s, r] of Object.entries(e)) {
      const e = `${t.id.toString()}:${s}`;
      this.#p.set(e, r);
      const o = t[s];
      void 0 !== o && (this[r] = o);
      const n = this.#r.properties[r];
  ...
  #v(t, e, s) {
    ...
    let r = typeof s;
    if ('object' === r) {
      const {constructor: o} = s;
      (r = o.name),
        o !== e &&
          this.#M(null, t, `was set to a ${r}, but must be a ${e.name}`);
    }
...
export class State {
  ...
  addListener(t, e = []) {
    const s = new Set(e);
    this.#D.push({listener: t, propertySet: s});
  }
  addProperty(t, e) {
    Object.defineProperty(this, t, {
      enumerable: !0,
  ...
  #B(t, e, s) {
    for (const {listener: r, propertySet: o} of this.#D)
      (o && !o.has(t)) || r.changed(this.#H, t, e, s);
  }
  removeListener(t) {
