...
Transcript windows do not support copy and paste, but Workspace windows do.
...
- The first (top) row contains four panes for displaying and operating on
  ...
  over over the list and type the letter.
...
- The third row contains a series of buttons that can be clicked to
  ...
  Click the "revert" button to revert the method code
  ...
  When there is no comment, it displays "Has no comment".

  Selecting "prettyPrint" displays the code for a method in a
  ...
  This could be useful for evaluating the efficiency of the code.
...
open a Hierarchy Browser by selecting a class
...
This is a contrived example because it would be better
...
which is a subclass of `Exception` that is resumable.
...
Occasionally the `Morph` for a thumbnail will not be visible.
To delete it:
