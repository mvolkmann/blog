...
Some errors indicate that they occurred while processing a generated file
with one of the following extensions:

- `.aux`: auxiliary file used to create the `.lof`, `.lot`, and `.toc` files
- `.glo`: glossary data; written by `\glossary` commands
- `.idx`: generated by `\index` commands
- `.ind`: index data written by the `\makeindex` command
  and read by the `\printindex` command
- `.lof`: list of figures; read by the `\listoffigures` command
- `.lot`: list of tables; read by the `\listoftables` command
- `.toc`: table of contents data; read by the `\tableofcontents` command
