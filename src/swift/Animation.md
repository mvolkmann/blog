...
Phases define discrete states that are provided to a view one at a time.
...
```swift
// The phases here are false and true.
...
Rather than phases represented by primitive values,
they are represented with enum cases.
The enum has methods that return phase-based values
...
Also see `KeyframeTimeline` to evaluate a set of `KeyframeTrack` instances
at a given time within the longest duration.
This can be used along with Swift Charts to plot timelines.
