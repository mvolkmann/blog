...
see the `checked` property in `src/toggle-switch.js`.
The component defined in `src/binding-demo.js`
listens for that event, as does the `script` in `src/index.html`.
...
Check out the `src` directory in the
